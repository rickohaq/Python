...
for DEVICE in MY_DEVICE:
    with EOSDriver(**DEVICE) as conn: #EOSDriver buat enable dan conf t
        response1 = conn.send_configs(["interface mana1", "description this desc configured by scrapli" ])
        response2 = conn.send_configs(["ip routing", "router ospf 1", "network  10.0.0.0  0.255.255.255 area 0"])
